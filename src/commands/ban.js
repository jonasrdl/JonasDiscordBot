module.exports = {
  ...
  execute(client, message, args, Discord) {
    ...
    const reason = args[1] + args[2] + args[3];

    let embed = new Discord.MessageEmbed().setTitle("Action: Ban").setDescription(`Banned ${target} (${target.id})`).setDescription(`Reason: ${args[1]} ${args[2]} ${args[3]}`).setColor("#ff2050").setFooter(`Banned by ${message.author.username}`);
